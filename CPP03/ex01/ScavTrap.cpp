...
ScavTrap::ScavTrap(void) : _hp(100), _maxhp(100), _energy(50), _maxenergy(50), _level(1), _melee(20), _ranged(15), _armor(3)
{
	std::cout << "[GLaDOS (unnamed)] (being constructed) " << this->_deploying[rand() % 6] << std::endl;
}

ScavTrap::ScavTrap(std::string name) : _hp(100), _maxhp(100), _energy(50), _maxenergy(50), _level(1), _name(name), _melee(20), _ranged(15), _armor(3)
{
	std::cout << "[GLaDOS " << this->_name << "] (being constructed) " << this->_deploying[rand() % 6] << std::endl;
}
...
ScavTrap	&ScavTrap::operator=(ScavTrap &rhs)
{
	...
	std::cout << "[GLaDOS " << this->_name << "] (being assigned) " << this->_equal[rand() % 5] << std::endl;
	return (*this);
}

ScavTrap::~ScavTrap(void)
{
	std::cout << "[GLaDOS " << this->_name << "] (dying) " << this->_death[rand() % 9] << std::endl;
}

void	ScavTrap::rangedAttack(std::string const &target)
{
	std::cout << "[GLaDOS " << this->_name << "] (while attacking " << target << ") " << this->_attack[rand() % 17] << " (-R" << this->_ranged << ")" << std::endl;
}

void	ScavTrap::meleeAttack(std::string const &target)
{
	std::cout << "[GLaDOS " << this->_name << "] (while attacking " << target << ") " << this->_attack[rand() % 17] << " (-M" << this->_melee << ")" << std::endl;
}
...
void	ScavTrap::takeDamage(unsigned int amount)
{
	...
	std::cout << "[GLaDOS " << this->_name << "] (taking damage) " << this->_hurt[rand() % 22] << " (-" << oldhp - this->_hp << ")" << std::endl;
}
...
void	ScavTrap::challengeNewcomer(std::string const &target)
{
	if (this->_energy >= 25)
	{
		std::cout << "[GLaDOS " << this->_name << "] (while challenging " << target << ") " << this->_attack[rand() % 17] << " (-" << (rand() % ((this->_melee + this->_ranged) / 2)) << ")" << std::endl;
		this->_energy -= 25;
	}
	else
...
std::string	ScavTrap::_deploying[6] =
{
	"Hello?",
	"Is anyone there?",
	"Oh! (surprised)",
...
std::string	ScavTrap::_attack[17] =
{
